...
Сервис для работы с Microsoft OAuth2 (Azure AD).
"""
...
class MicrosoftOAuthError(ServiceError):
    """Ошибки Microsoft OAuth."""
    pass


class MicrosoftOAuthService(BaseService):
    """Сервис для работы с Microsoft OAuth2."""

    def get_service_name(self) -> str:
        return "MicrosoftOAuthService"

    def __init__(self):
        self.client_id = settings.microsoft_oauth.client_id if hasattr(settings, 'microsoft_oauth') else None
        self.client_secret = settings.microsoft_oauth.client_secret if hasattr(settings, 'microsoft_oauth') else None
        self.redirect_uri = settings.microsoft_oauth.redirect_uri if hasattr(settings, 'microsoft_oauth') else None
        self.tenant_id = getattr(settings.microsoft_oauth, 'tenant_id', 'common') if hasattr(settings, 'microsoft_oauth') else 'common'
    ...
    def is_enabled(self) -> bool:
        """Проверяет, включен ли Microsoft OAuth."""
    ...
    async def get_authorization_url(
        ...
        """
        Получить URL авторизации Microsoft.
    ...
    async def handle_callback(
        ...
        """
        Обработать callback от Microsoft.
        
        Args:
            db: Сессия базы данных
            code: Код авторизации от Microsoft
        ...
        try:
            ...
            raise MicrosoftOAuthError(f"Microsoft OAuth callback failed: {str(e)}")
    ...
    async def _get_user_info(self, access_token: str) -> Dict[str, Any]:
        """Получить информацию о пользователе от Microsoft Graph."""
        headers = {
    ...
    async def _find_or_create_user(self, db: AsyncSession, user_info: Dict[str, Any]) -> User:
        """Найти или создать пользователя на основе данных Microsoft."""
        ...
        if user:
            ...
            if not user.name and user_info.get("displayName"):
        ...
        # Построить полное имя
        given_name = user_info.get("givenName", "")
        surname = user_info.get("surname", "")
        display_name = user_info.get("displayName", "")
        
        name = display_name or f"{given_name} {surname}".strip() or username
        ...
        user_data = {
            ...
            "username": username,
            "is_active": True,
            "is_email_verified": True,  # Microsoft emails считаем подтвержденными
        }
    ...
    async def link_account(
        ...
        """
        Привязать Microsoft аккаунт к существующему пользователю.
        
        Args:
            db: Сессия базы данных
            user: Пользователь для привязки
            access_token: Access token от Microsoft
        """
        if not self.is_enabled:
            raise MicrosoftOAuthError("Microsoft OAuth is not configured")

        try:
            # Получить информацию о пользователе Microsoft
            user_info = await self._get_user_info(access_token)
            
            # TODO: Сохранить связь между пользователем и Microsoft аккаунтом
            # Это требует дополнительной модели для OAuth связей
            email = user_info.get("mail") or user_info.get("userPrincipalName")
            logger.info(f"Microsoft account linked for user {user.id}: {email}")
            
        except Exception as e:
            logger.error(f"Failed to link Microsoft account: {e}")
            raise MicrosoftOAuthError(f"Failed to link Microsoft account: {str(e)}")

    async def unlink_account(self, db: AsyncSession, user: User) -> None:
        """
        Отвязать Microsoft аккаунт от пользователя.
        ...
        try:
            # TODO: Удалить связь между пользователем и Microsoft аккаунтом
            # Это требует дополнительной модели для OAuth связей
            logger.info(f"Microsoft account unlinked for user {user.id}")
            
        except Exception as e:
            logger.error(f"Failed to unlink Microsoft account: {e}")
            raise MicrosoftOAuthError(f"Failed to unlink Microsoft account: {str(e)}")
